...
type Args struct {
	Entity string         `json:"entity" jsonschema:"audio | image | o_auth2_application | o_auth2_key_info | o_auth2_token"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.OpenverseSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "openverse_list",
		Description: "List records from Openverse. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "openverse_load",
		Description: "Load a single record from Openverse. " +
...
func runOp(client *sdk.OpenverseSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.OpenverseSDK, name string) (sdk.OpenverseEntity, error) {
	switch strings.ToLower(name) {
	case "audio":
		return client.Audio(nil), nil
	case "image":
		return client.Image(nil), nil
	case "o_auth2_application":
		return client.OAuth2Application(nil), nil
	case "o_auth2_key_info":
		return client.OAuth2KeyInfo(nil), nil
	case "o_auth2_token":
		return client.OAuth2Token(nil), nil
