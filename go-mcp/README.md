...
[MCP](https://modelcontextprotocol.io) server exposing the Openverse SDK as
two agent tools — `openverse_list` and `openverse_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/openverse-mcp)
make build

# 2. Provide credentials via the environment
export OPENVERSE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user openverse \
  -- /absolute/path/to/openverse-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./openverse-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// openverse_list: first page of records
{ "entity": "audio" }
{ "entity": "audio", "query": { } }

// openverse_load: one record by id
{ "entity": "audio", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/openverse-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user openverse \
     -- "$PWD"/dist/*/openverse-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `openverse_list` and `openverse_load` tools now appear
   in new sessions. Ask the agent to *"list audio using openverse"*
   and it calls `openverse_list` with `{"entity":"audio"}`.
...
```sh
export OPENVERSE_APIKEY=sk_live_xxx            # API key
export OPENVERSE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `openverse_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `openverse_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPENVERSE_APIKEY` | API key sent with every request. |
| `OPENVERSE_BASE` | Optional override of the API base URL. |
...
```sh
./openverse-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"openverse_load","arguments":{"entity":"audio","query":{"id":1}}}}'
```
