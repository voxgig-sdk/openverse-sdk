...
describe("AudioEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Audio(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Audio(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Audio(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = audio_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "audio." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set OPENVERSE_TEST_AUDIO_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local audio_ref01_ent = client:Audio(nil)
    local audio_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local audio_ref01_match = {}

    local audio_ref01_list_result, err = audio_ref01_ent:list(audio_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(audio_ref01_list_result)
...
function audio_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/audio/AudioTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read audio test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("OPENVERSE_TEST_AUDIO_ENTID")
