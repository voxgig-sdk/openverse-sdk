...
describe("OAuth2ApplicationEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:OAuth2Application(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = o_auth2_application_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "o_auth2_application." .. _op, _live and "live" or "unit")
    ...
    -- CREATE
    local o_auth2_application_ref01_ent = client:OAuth2Application(nil)
    local o_auth2_application_ref01_data = helpers.to_map(vs.getprop(
...
function o_auth2_application_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/o_auth2_application/OAuth2ApplicationTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read o_auth2_application test data: " .. entity_data_file)
  end
