...
describe("AudioDirect", function()
  it("should direct-list-audio", function()
    local setup = audio_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-audio", setup.live and "live" or "unit")
  ...
  it("should direct-load-audio", function()
    local setup = audio_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-audio", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function audio_direct_setup(mockres)
