...
local AudioEntity = {}
AudioEntity.__index = AudioEntity


function AudioEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AudioEntity)
  self._name = "audio"
  self._client = client
...
function AudioEntity:get_name()
  return self._name
end


function AudioEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AudioEntity.new(self._client, opts)
end
...
function AudioEntity:mark_deleted()
  self._deleted = true
end


function AudioEntity:deleted()
  return true == self._deleted
end


function AudioEntity:data_set(args)
  if args ~= nil then
...
function AudioEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AudioEntity:match_set(args)
  if args ~= nil then
...
function AudioEntity:match_get()
...
function AudioEntity:stream(action, args, callopts)
...
function AudioEntity:_run_op(ctx, post_done)
  local utility = self._utility
