-- Openverse SDK OAuth2Token entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local OAuth2TokenEntity = {}
OAuth2TokenEntity.__index = OAuth2TokenEntity


function OAuth2TokenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OAuth2TokenEntity)
  self._name = "o_auth2_token"
  self._client = client
...
function OAuth2TokenEntity:get_name()
  return self._name
end


function OAuth2TokenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OAuth2TokenEntity.new(self._client, opts)
end
...
function OAuth2TokenEntity:mark_deleted()
  self._deleted = true
end


function OAuth2TokenEntity:deleted()
  return true == self._deleted
end


function OAuth2TokenEntity:data_set(args)
  if args ~= nil then
...
function OAuth2TokenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OAuth2TokenEntity:match_set(args)
  if args ~= nil then
...
function OAuth2TokenEntity:match_get()
...
function OAuth2TokenEntity:stream(action, args, callopts)
...
---@param reqdata OAuth2TokenCreateData
---@param ctrl? table
---@return OAuth2Token
---@return string? err
function OAuth2TokenEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function OAuth2TokenEntity:_run_op(ctx, post_done)
  local utility = self._utility
