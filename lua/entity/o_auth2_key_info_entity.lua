-- Openverse SDK OAuth2KeyInfo entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local OAuth2KeyInfoEntity = {}
OAuth2KeyInfoEntity.__index = OAuth2KeyInfoEntity


function OAuth2KeyInfoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OAuth2KeyInfoEntity)
  self._name = "o_auth2_key_info"
  self._client = client
...
function OAuth2KeyInfoEntity:get_name()
  return self._name
end


function OAuth2KeyInfoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OAuth2KeyInfoEntity.new(self._client, opts)
end
...
function OAuth2KeyInfoEntity:mark_deleted()
  self._deleted = true
end


function OAuth2KeyInfoEntity:deleted()
  return true == self._deleted
end


function OAuth2KeyInfoEntity:data_set(args)
  if args ~= nil then
...
function OAuth2KeyInfoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OAuth2KeyInfoEntity:match_set(args)
  if args ~= nil then
...
function OAuth2KeyInfoEntity:match_get()
...
function OAuth2KeyInfoEntity:stream(action, args, callopts)
...
function OAuth2KeyInfoEntity:_run_op(ctx, post_done)
  local utility = self._utility
