...
class AudioEntity
  ...
  def make
    opts = @_entopts.dup
    AudioEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = OpenverseHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Audio, Hash] the current Audio data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = OpenverseHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Audio fields)
  def match_get
  ...
  # Load a single Audio.
  #
  # @param reqmatch [AudioLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Audio.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Audio, Hash] the loaded Audio; raises OpenverseError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = OpenverseHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [AudioListMatch, Hash, nil] match filter (any subset of
  #   Audio fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Audio>, Array] the matching Audio items; raises OpenverseError on failure
  ...
  # Create a new Audio.
  #
  # @param reqdata [AudioCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Audio, Hash] the created Audio; raises OpenverseError on failure
