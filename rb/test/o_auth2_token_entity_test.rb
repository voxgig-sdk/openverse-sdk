...
class OAuth2TokenEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set OPENVERSE_TEST_O_AUTH2_TOKEN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    o_auth2_token_ref01_data_result = o_auth2_token_ref01_ent.create(o_auth2_token_ref01_data, nil)
    o_auth2_token_ref01_data = Helpers.to_map(o_auth2_token_ref01_data_result.respond_to?(:data_get) ? o_auth2_token_ref01_data_result.data_get : o_auth2_token_ref01_data_result)
...
def o_auth2_token_basic_setup(extra)
  ...
  entid_env_raw = ENV["OPENVERSE_TEST_O_AUTH2_TOKEN_ENTID"]
