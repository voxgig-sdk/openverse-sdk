...
class OAuth2ApplicationEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = o_auth2_application_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "o_auth2_application." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set OPENVERSE_TEST_O_AUTH2_APPLICATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    o_auth2_application_ref01_ent = client.OAuth2Application(nil)
    ...
    o_auth2_application_ref01_data = Helpers.to_map(o_auth2_application_ref01_data_result.respond_to?(:data_get) ? o_auth2_application_ref01_data_result.data_get : o_auth2_application_ref01_data_result)
    assert !o_auth2_application_ref01_data.nil?

  end
end

def o_auth2_application_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "o_auth2_application", "OAuth2ApplicationTestData.json")
  ...
  entid_env_raw = ENV["OPENVERSE_TEST_O_AUTH2_APPLICATION_ENTID"]
