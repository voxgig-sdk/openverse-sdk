...
module OpenverseUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OpenverseResponse.new({})
    ...
    if fetch_err
      ...
      response = OpenverseResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpenverseResponse.new(fetched)
    else
