...
from openverse_sdk.openverse_types import (
    OAuth2KeyInfo,
...
class OAuth2KeyInfoEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return OAuth2KeyInfoEntity(self._client, opts)
    ...
    def data_get(self) -> OAuth2KeyInfo:
    ...
    def match_get(self) -> OAuth2KeyInfo:
    ...
    def load(self, reqmatch=None, ctrl=None) -> OAuth2KeyInfo:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.OAuth2KeyInfo().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
