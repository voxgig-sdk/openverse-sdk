...
from openverse_sdk.core.helpers import get_ctx_prop, to_map
...
class OpenverseContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, OpenverseSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, OpenverseResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, OpenverseResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return OpenverseError(code, msg, self)
