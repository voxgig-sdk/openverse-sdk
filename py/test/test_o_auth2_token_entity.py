...
class TestOAuth2TokenEntity:

    def test_should_create_instance(self):
        testsdk = OpenverseSDK.test(None, None)
        ent = testsdk.OAuth2Token(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _o_auth2_token_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "o_auth2_token." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set OPENVERSE_TEST_O_AUTH2_TOKEN_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        o_auth2_token_ref01_ent = client.OAuth2Token(None)
        o_auth2_token_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.o_auth2_token"), "o_auth2_token_ref01"))

        o_auth2_token_ref01_data = helpers.to_map(runner.entity_data(o_auth2_token_ref01_ent.create(o_auth2_token_ref01_data, None)))
        assert o_auth2_token_ref01_data is not None



def _o_auth2_token_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/o_auth2_token/OAuth2TokenTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "OPENVERSE_TEST_O_AUTH2_TOKEN_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("OPENVERSE_TEST_O_AUTH2_TOKEN_ENTID"))
