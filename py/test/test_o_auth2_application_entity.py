...
class TestOAuth2ApplicationEntity:

    def test_should_create_instance(self):
        testsdk = OpenverseSDK.test(None, None)
        ent = testsdk.OAuth2Application(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _o_auth2_application_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "o_auth2_application." + _op, "live" if _live else "unit")
            if _skip:
        ...
        # CREATE
        o_auth2_application_ref01_ent = client.OAuth2Application(None)
...
def _o_auth2_application_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/o_auth2_application/OAuth2ApplicationTestData.json")
