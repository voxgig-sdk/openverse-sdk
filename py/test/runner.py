...
class OpenverseTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OpenverseTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OpenverseTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OpenverseTestRunner.getenv("OPENVERSE_TEST_LIVE")
        override = OpenverseTestRunner.getenv("OPENVERSE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OpenverseTestRunner.getenv(key)
        ...
        explain = OpenverseTestRunner.getenv("OPENVERSE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["OPENVERSE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if OpenverseTestRunner._test_control is not None:
            return OpenverseTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OpenverseTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OpenverseTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OpenverseTestRunner.load_test_control()
...
def load_env_local():
    OpenverseTestRunner.load_env_local()


def env_override(m):
    return OpenverseTestRunner.env_override(m)


def entity_data(v):
    return OpenverseTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OpenverseTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OpenverseTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OpenverseTestRunner.load_test_control()


def live_client_options():
    return OpenverseTestRunner.live_client_options()


def live_delay_ms():
    return OpenverseTestRunner.live_delay_ms()
