...
//     (\`new OpenverseSDK(...)\` / \`OpenverseSDK.test(...)\` become
//     \`OpenverseSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"audio":{"test01":{"id":"test01"}},"image":{"test01":{"id":"test01"}},"o_auth2_application":{"test01":{"id":"test01"}},"o_auth2_key_info":{"test01":{"id":"test01"}},"o_auth2_token":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OpenverseSDK(...)\` and any
// \`OpenverseSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OpenverseSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OpenverseSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
