...
describe('OAuth2KeyInfoDirect', async () => {
  ...
  test('direct-load-o_auth2_key_info', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-o_auth2_key_info', setup.live)) return
