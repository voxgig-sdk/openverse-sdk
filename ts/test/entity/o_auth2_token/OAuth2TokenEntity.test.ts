...
describe('OAuth2TokenEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'o_auth2_token.' + op, live)) return
    }
    ...
    o_auth2_token_ref01_data = await o_auth2_token_ref01_ent.create(o_auth2_token_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['OPENVERSE_TEST_O_AUTH__TOKEN_ENTID']
