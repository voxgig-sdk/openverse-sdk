...
(0, node_test_1.describe)('AudioDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.OpenverseSDK({
    ...
    (0, node_test_1.test)('direct-load-audio', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-audio', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["identifier01"]))
            return;
    ...
    (0, node_test_1.test)('direct-list-audio', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-audio', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.OPENVERSE_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.OpenverseSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.OPENVERSE_APIKEY,
        }));
        let idmap = env['OPENVERSE_TEST_AUDIO_ENTID'];
    ...
    const client = new __1.OpenverseSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=AudioDirect.test.js.map