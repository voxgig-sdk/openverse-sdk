...
The PHP SDK for the Openverse API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Audio(null)->list(null, null);
...
```php
[$result, $err] = $client->Audio(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
```php
// Create
[$created, $_] = $client->Audio(null)->create(["name" => "Example"], null);
...
[$result, $err] = $client->Openverse(null)->load(
    ["id" => "test01"], null
);
