...
class AudioEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AudioEntity($this->_client, $opts);
    }

    /**
     * @param Audio|array $args Audio data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = OpenverseHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Audio|array The current Audio data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Audio fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = OpenverseHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Audio fields).
     */
     ...
     * Load a single Audio.
     *
     * @param AudioLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AudioLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Audio|array The loaded Audio as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = OpenverseHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Audio items matching the given filter.
     *
     * @param AudioListMatch|array|null $reqmatch Match filter (any subset
     *   of Audio fields) as an assoc-array; AudioListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Audio[]|array A list of Audio items as assoc-arrays at
     ...
     * Create a new Audio.
     *
     * @param AudioCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed AudioCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Audio|array The created Audio as an assoc-array at the
     *   SDK boundary; throws OpenverseError on failure (item-5 convention).
     */
