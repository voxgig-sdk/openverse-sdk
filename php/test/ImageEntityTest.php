...
class ImageEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set OPENVERSE_TEST_IMAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        [$image_ref01_data_result, $err] = $image_ref01_ent->create($image_ref01_data, null);
        $this->assertNull($err);
...
function image_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["image01", "image02", "image03", "identifier01"] as $k) {
    ...
    $entid_env_raw = getenv("OPENVERSE_TEST_IMAGE_ENTID");
