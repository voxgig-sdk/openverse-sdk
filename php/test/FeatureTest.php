...
class FtCtrl extends OpenverseControl
{
...
class FtClient
{
    ...
    private OpenverseUtility $utility;

    public function __construct(OpenverseUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpenverseUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpenverseContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpenverseContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpenverseContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpenverseConfig::make_feature($name);
    ...
    private static function buildUrl(OpenverseSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpenverseError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpenverseResponse($fetched);
        }
    ...
    private function fail(OpenverseContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpenverseContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
