...
class OAuth2KeyInfoEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = OpenverseSDK::test(null, null);
        $ent = $testsdk->OAuth2KeyInfo(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = o_auth2_key_info_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "o_auth2_key_info." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $o_auth2_key_info_ref01_data = null;
        if (count($o_auth2_key_info_ref01_data_raw) > 0) {
...
function o_auth2_key_info_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/o_auth2_key_info/OAuth2KeyInfoTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["o_auth2_key_info01", "o_auth2_key_info02", "o_auth2_key_info03"] as $k) {
