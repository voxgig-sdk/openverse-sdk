...
class OAuth2TokenEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = OpenverseSDK::test(null, null);
        $ent = $testsdk->OAuth2Token(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = o_auth2_token_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "o_auth2_token." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        // CREATE
        $o_auth2_token_ref01_ent = $client->OAuth2Token(null);
        $o_auth2_token_ref01_data = Helpers::to_map(Vs::getprop(
            Vs::getpath($setup["data"], "new.o_auth2_token"), "o_auth2_token_ref01"));

        [$o_auth2_token_ref01_data_result, $err] = $o_auth2_token_ref01_ent->create($o_auth2_token_ref01_data, null);
        $this->assertNull($err);
...
function o_auth2_token_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/o_auth2_token/OAuth2TokenTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["o_auth2_token01", "o_auth2_token02", "o_auth2_token03"] as $k) {
