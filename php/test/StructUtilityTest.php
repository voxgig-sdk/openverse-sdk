...
// Openverse SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = OpenverseOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(OpenverseOmni::structfix($now, Struct::undef()))
                        !== json_encode(OpenverseOmni::structfix($val, Struct::undef()))) {
