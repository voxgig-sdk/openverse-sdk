...
{
    private static function utility(): OpenverseUtility
    {
        return new OpenverseUtility();
    }
    ...
    private static function ctx(array $over = []): OpenverseContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OpenverseContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OpenverseOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OpenverseError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OpenverseOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OpenverseMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OpenverseOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OpenverseMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OpenverseOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OpenverseMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenverseSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OpenverseMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OpenverseResponse([]),
            'result' => new OpenverseResult([]),
        ]);
        [, $err] = OpenverseMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OpenverseSpec([]),
            'response' => null,
            'result' => new OpenverseResult([]),
        ]);
        [, $err] = OpenverseMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OpenverseSpec(['step' => 's']),
            'response' => new OpenverseResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OpenverseResult([]),
        ]);
        [, $err] = OpenverseMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenverseSpec(['step' => 's']),
            'response' => new OpenverseResponse(self::resp(200, ['v' => 1])),
            'result' => new OpenverseResult([]),
        ]);
        [, $err] = OpenverseMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenverseSpec(['step' => 's']),
            'response' => new OpenverseResponse(self::resp(200, ['v' => 2])),
            'result' => new OpenverseResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OpenverseMakeResponse::call($ctx);
    ...
    {
        $preset = new OpenverseResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OpenverseSpec([]),
            'response' => new OpenverseResponse([]),
            'result' => new OpenverseResult([]),
        ...
        [$got, $err] = OpenverseMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OpenverseResult([])]);
        [, $err] = OpenverseMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OpenverseSpec([]), 'result' => null]);
        [, $err] = OpenverseMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenverseSpec(['step' => 's']),
            'result' => new OpenverseResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OpenverseOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenverseMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenverseSpec(['step' => 's']),
            'result' => new OpenverseResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OpenverseOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenverseMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenverseResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OpenverseSpec([]),
            'result' => new OpenverseResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OpenverseMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenverseMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OpenverseError('boom', 'boom');
        $utility->fetcher = function (OpenverseContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenverseSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OpenverseMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OpenverseContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OpenverseMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OpenverseResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OpenverseContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenverseSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OpenverseMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OpenverseResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OpenverseSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OpenverseMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OpenverseMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OpenverseMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OpenverseResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OpenverseDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OpenverseResult(['ok' => false])]);
        $this->expectException(OpenverseError::class);
        OpenverseDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OpenverseResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OpenverseMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OpenverseMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OpenverseBaseFeature();
        $b = new OpenverseBaseFeature();
        OpenverseFeatureAdd::call($ctx, $a);
        OpenverseFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OpenverseBaseFeature
    {
        $f = new OpenverseBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OpenverseFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OpenverseContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OpenverseSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenversePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OpenverseResponse(['headers' => null]),
            'result' => new OpenverseResult([]),
        ]);
        OpenverseResultHeaders::call($ctx);
