...
class OpenverseRetryFeature extends OpenverseBaseFeature
{
    ...
    public function init(OpenverseContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OpenverseContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OpenverseContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OpenverseContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
