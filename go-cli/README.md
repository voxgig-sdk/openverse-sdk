...
boru-driven command-line client **and** interactive REPL for the Openverse
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/openverse-cli)
make build
...
./openverse-cli load 1 audio            # {id:1} shorthand
./openverse-cli load '{id:1}' audio       # explicit match map
./openverse-cli list image

# 5. Override the API base URL for a single call
OPENVERSE_BASE=https://api.example.com ./openverse-cli list audio

# 6. No arguments -> interactive REPL
./openverse-cli
openverse> list audio
openverse> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/openverse-cli
   ```
...
```sh
./openverse-cli load 1 audio          # scalar shorthand for {id:1}
./openverse-cli load '{id:1}' audio     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `openverse>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `audio`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/openverse-cli`. |
...
(`./openverse-cli <expr>`) and interactively (the REPL), and expressions compose the
