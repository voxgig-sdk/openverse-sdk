// Command Openverse-cli is a CLI + REPL for the Openverse SDK,
...
const prompt = "openverse"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "audio image o_auth2_application o_auth2_key_info o_auth2_token"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      OPENVERSE_APIKEY (api key), OPENVERSE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: OPENVERSE_APIKEY carries the API key and
	// OPENVERSE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OPENVERSE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OPENVERSE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOpenverseSDK(opts)
