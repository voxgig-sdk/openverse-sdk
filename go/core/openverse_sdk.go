...
func (sdk *OpenverseSDK) OptionsMap() map[string]any {
...
func (sdk *OpenverseSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *OpenverseSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *OpenverseSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *OpenverseSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *OpenverseSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *OpenverseSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("OpenverseSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *OpenverseSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *OpenverseSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("OpenverseSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
