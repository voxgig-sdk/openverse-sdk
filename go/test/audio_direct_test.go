...
func TestAudioDirect(t *testing.T) {
	t.Run("direct-list-audio", func(t *testing.T) {
		setup := audioDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-audio", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-audio", func(t *testing.T) {
		setup := audioDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-audio", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type audioDirectSetupResult struct {
...
func audioDirectSetup(mockres any) *audioDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["OPENVERSE_TEST_AUDIO_ENTID"]; ok {
		...
		return &audioDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &audioDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
